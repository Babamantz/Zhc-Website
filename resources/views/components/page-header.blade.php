...
@php
    ...
    $segments = request()->segments(); // e.g. ['services', 'ict', 'hosting']
    ...
    $pageTitle = $title 
        ? __($title) 
        : (count($breadcrumbs) ? array_key_last($breadcrumbs) : __('Home'));
@endphp

<section class="bg-gray-altinative h-32 border-b-2 lg:px-8 px-4">
    <div class="container mx-auto">
        <div class="flex flex-col justify-between py-12 md:flex-row">
            
            <div class="mx-2">
                <span class="font-bold text-base md:text-xl uppercase">
                    {{ $pageTitle }}
                </span>
            </div>
                ...
                <ul class="breadcrumb font-semibold text-md pl-0 flex gap-2">
                    ...
                    @foreach ($breadcrumbs as $label => $url)
                        <li class="breadcrumb-item capitalize">
                            ...
                            @else
                                <span class="text-gray-600">{{ __($label) }}</span>
                            @endif
                        </li>
                    @endforeach
                </ul>
